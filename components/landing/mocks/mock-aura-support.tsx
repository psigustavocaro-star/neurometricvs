...
export function MockAuraSupport() {
    const t = useTranslations('Mocks.AuraSupport')

    return (
        <div className="w-full max-w-xl self-center h-[500px] bg-white dark:bg-slate-950 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 flex flex-col shadow-2xl overflow-hidden ring-1 ring-slate-900/5 relative">
            {/* Clinical Header */}
            ...
            {/* Chat Area */}
            <div className="flex-1 p-6 space-y-6 overflow-hidden relative bg-slate-50/50 dark:bg-slate-900/10">
                {/* User query 1 */}
                ...
                >
                    <div className="w-8 h-8 rounded-full bg-teal-500 flex items-center justify-center shrink-0 shadow-md transform -translate-y-2">
                        <Bot className="h-4 w-4 text-white" />
                    </div>
                    <div className="bg-teal-600 text-white text-xs p-5 rounded-2xl rounded-tl-none max-w-[85%] shadow-lg shadow-teal-500/10 border border-teal-500/20 leading-relaxed relative">
                        {t('response_1')}
                ...
                <div className="absolute bottom-4 left-6 right-6">
                    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-3 flex items-center justify-between shadow-lg">
                        <span className="text-xs text-slate-400 ml-2">Responder a Aura...</span>
