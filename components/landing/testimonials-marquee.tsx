...
export function TestimonialsMarquee() {
    ...
    return (
        <div className="w-full overflow-hidden relative [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]">
            ...
            <div className="flex w-max animate-marquee-slow gap-8 py-4 hover:[animation-play-state:paused]">
                {/* Original Set */}
...
function TestimonialCard({ t }: { t: { name: string, role: string, text: string, img: string } }) {
    return (
        <div className="w-[290px] md:w-[400px] flex-shrink-0">
            <Card className="shadow-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-primary/30 transition-colors h-full">
                <CardContent className="p-6 flex flex-col gap-4 h-full">
                    <div className="flex gap-1 text-amber-400">
                        {[1, 2, 3, 4, 5].map(s => <Star key={s} className="w-4 h-4 fill-current" />)}
                    </div>
                    <p className="text-slate-600 dark:text-slate-300 relative flex-1 leading-relaxed">
                        "{t.text}"
                    </p>
